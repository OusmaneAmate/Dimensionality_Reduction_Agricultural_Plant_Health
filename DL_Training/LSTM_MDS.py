...
def LSTMClassificationModel(x, y):
 
    mds = MDS(n_components=22)
    x_mds = mds.fit_transform(x_std)
    print(f"🧪 MDS: {x.shape[1]} ➝ {x_mds.shape[1]} dimensions conservées")

    
    scaler_minmax = MinMaxScaler()
    x_scaled = scaler_minmax.fit_transform(x_mds)
...
for crop in DATASETS:
    ...
    for run in range(N_RUNS):
        ...
        acc, prec, f1 = LSTMClassificationModel(x, y)
        acc_list.append(acc)
...
def LSTMRegressionModel(x, y, run_number=None):
    ...
    mds = MDS(n_components=22, n_init=1, random_state=42, n_jobs=-1)
    x_mds = mds.fit_transform(x_std)
    print(f"🧪 MDS: {x.shape[1]} ➝ {x_mds.shape[1]} dimensions")

    
    scaler = MinMaxScaler()
    x_scaled = scaler.fit_transform(x_mds)
