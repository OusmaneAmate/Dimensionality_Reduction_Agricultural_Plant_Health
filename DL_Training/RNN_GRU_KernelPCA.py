...
class GRUClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim=16, num_layers=1, dropout=0.3):
...
def train_gru_classification_model(x, y, n_runs=10):
    ...
    for run in range(n_runs):
        ...
        model = GRUClassifier(input_dim=x_Kpca.shape[1]).to(device)
...
def train_gru_regression_model(x, y, n_runs=10):
    ...
    for run in range(n_runs):
        ...
        kpca = KernelPCA(n_components=22)
        X_kpca = kpca.fit_transform(x_scaled)
        print(f"🧪 KPCA: {x.shape[1]} ➝ {X_kpca.shape[1]} dimensions conservées")
        ...
        model = GRURegressor(input_dim=X_kpca.shape[1]).to(device)
