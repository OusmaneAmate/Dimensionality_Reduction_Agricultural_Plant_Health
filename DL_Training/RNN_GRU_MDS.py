...
def train_gru_classification_model(x, y, n_runs=10):
    ...
    for run in range(n_runs):
        ...
        x_train, x_test, y_train, y_test = train_test_split(x_mds, y, test_size=0.2, shuffle=True)
        ...
        model = GRUClassifier(input_dim=x_mds.shape[1]).to(device)
...
def train_gru_regression_model(x, y, n_runs=10):
    ...
    for run in range(n_runs):
        ...
        model = GRURegressor(input_dim=X_mds.shape[1]).to(device)
